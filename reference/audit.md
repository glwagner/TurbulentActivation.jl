...
| Condensation sink | bin microphysics with injected NaCl; droplet number not stated for the trajectory run | `QLDot` field in the snapshots (condensation rate, g kg⁻¹ s⁻¹; `QP` is identically zero) | `analysis/reference_condensation.jl`: QLDot correlates at 0.96 with qᵛ⁺ 𝒮 over the interior of 19 snapshots and the regression gives a phase relaxation time τ ≈ 72 s, i.e. a very clean cloud (N r ≈ 44 m⁻², roughly 3–5 cm⁻³ of 15 μm droplets; LWC 0.06 g kg⁻¹ with a ≈ 40 s residence time). The chamber host must use this τ (`HOST_TAU=72`), not the 5 s used at first |
